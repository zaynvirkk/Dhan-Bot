...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "enable":
            ...
            (state / "status.json").write_text(json.dumps({"state":"ARMED_WAITING_SESSION","auto_live_armed":True,"software_verified":True,"writes":True}, sort_keys=True)+"\n", encoding="utf-8")
            print(json.dumps({"enabled": True, "mode": "AUTO_LIVE"}))
            return 0
        ...
        if args.command == "disarm":
            ...
            (Path(config["state_dir"]) / "status.json").write_text(json.dumps({"state":"ENTRY_HALTED","auto_live_armed":False,"reason":"operator_disarmed_new_entries","writes":False}, sort_keys=True)+"\n", encoding="utf-8")
            print(json.dumps({"disarmed": True, "new_entries": False}))
            return 0
        if args.command == "run":
